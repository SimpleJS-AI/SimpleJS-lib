...
class NeuralNetwork {
    ...
    bp(desired_input, desired_output) {
        let input = [...desired_input];  //Copy array
        if(desired_input.length !== this.input || desired_output.length !== this.output) return false;
        let [inputMatrix, hiddenMatrix, outputMatrix] = this.ff(input, true);
        let e_output = math.subtract(desired_output, outputMatrix);
...
class GeneticAlgorithm {
    ...
    _mutate(population) {              //TODO: instead of mutating randomly, tune values by adding/subtracting a small amount
        for (let i = 1; i < population.length-1; i++) {
            for (let j = 0; j < population[i].nn._wInputHidden.size()[0]; j++) {
                for (let k = 0; k < population[i].nn._wInputHidden.size()[1]; k++) {
                    if (Math.random() < .1) population[i].nn._wInputHidden._data[j][k] = Math.random() * 2 - 1; //TODO: create Variable Mutation Rate
                }
            }
            for (let j = 0; j < population[i].nn._wHiddenOutput.size()[0]; j++) {
                for (let k = 0; k < population[i].nn._wHiddenOutput.size()[1]; k++) {
                    if (Math.random() < .1) population[i].nn._wHiddenOutput._data[j][k] = Math.random() * 2 - 1;
                }
            }
        }
        return population;
